        ...
        if (token_id && web_hook_id) {
            ...
            return this._http.get<{ data: any }>(`/api/web-hook/${web_hook_id}`).pipe(
                ...
                mergeMap((webHook: WebHook) => {
                    let permissionInfo = '<div>该 Web Hook 需要以下权限</div><ul>' + webHook.permissions.map((permission_key) => {
                        return `<li>${PERMISSION_INFO[permission_key]}</li>`;
                    }).join('') + '</ul>';
                    let content = `<div>将要添加 ${webHook.name} 至您的账号。您可以随时删除该 Web Hook 。</div>`;
                    ...
                    dialogRef.componentInstance.title = '确定添加 Web Hook 吗？';
